...
def train_net(net,
    ...
    dir_checkpoint = 'checkpoints3d_norm_label_'+str(ratio)+'datalr'+str(lr)+'k'+str(coeff)+'/'
    if not os.path.exists(dir_checkpoint):
        os.mkdir(dir_checkpoint)
    writer = SummaryWriter(logdir='run3d_norm_label_' + str(ratio) + 'datalr' + str(lr) + 'k' + str(coeff) + '/',
                           comment='Linear')
...
def get_args():
    ...
    parser.add_option('-k', '--k', dest='coefficient', type='float',
                      default=200, help='seg:color loss')
