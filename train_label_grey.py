...
def train_net(net,
    ...
    dir_checkpoint = 'checkpoints_grey_label_'+str(ratio)+'datalr'+str(lr)+'k'+str(coeff)+'/'
    writer = SummaryWriter(logdir='run_label_grey_' + str(ratio) + 'datalr' + str(lr) + 'k' + str(coeff) + '/',
                           comment='Linear')
    ...
    d_val = d[:n_labeled]
    d_train = d[n_labeled:]
    train_set = KittiLoader(dir_img, dir_mask, d_train, ratio)
    train_loader = DataLoader(train_set, batch_size=batch_size,
                            shuffle=True, num_workers=1)
                            #pin_memory=True)
    n = len(train_set)

    valid_set = KittiLoader(dir_img, dir_mask, d_val,ratio)
    ...
    for epoch in range(epochs):
        ...
        for i, b in enumerate(train_loader):
            ...
            # print(weights)
            weights = np.array(weights, dtype = np.bool)
            weights.squeeze()
            # print(weights)
            mk = [a[0] for a in weights]
            print(mk)
        ...
        for i, b in enumerate(val_loader):
            ...
            print('{0:.4f} --- seg loss: {1:.6f}'.format(i * batch_size / n_val, loss.item()))  ###应该很多0
