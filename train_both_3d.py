...
def train_net(net,
    ...
    dir_checkpoint = 'checkpoints3d_norm_both_'+str(ratio)+'datalr'+str(lr)+'k'+str(coeff)+'/'
    if not os.path.exists(dir_checkpoint):
        os.mkdir(dir_checkpoint)
    writer = SummaryWriter(logdir='run3d_norm_both_' + str(ratio) + 'datalr' + str(lr) + 'k' + str(coeff) + '/',
                           comment='Linear')
    ...
    for epoch in range(epochs):
        ...
        for i, b in enumerate(train_loader):
            ...
            try:
                ...
                writer.add_scalar('Seg', loss/int(mk.sum()), total_steps)
        ...
        for i, b in enumerate(val_loader):
            ...
            try:
                ...
                print('{0:.4f} --- seg val loss: {1:.6f}'.format(i * batch_size / n, loss.item()))  ###应该很多0
            except AttributeError:
                pass
            if total_val_steps % 100 == 0:
                out = masks_pred[0].cpu()

                out = out.detach().numpy().transpose(1, 2,3, 0)
                out = np.asarray(np.argmax(out, axis=3), dtype=np.int)
                out = out.squeeze()
                out = out[:,32,:]
...
def get_args():
    ...
    parser.add_option('-k', '--k', dest='coefficient', type='float',
                      default=100, help='seg:color loss')
    parser.add_option('-r', '--r', dest='rat', type='float',
                      default=0.01, help='lab:nonlab')
...
if __name__ == '__main__':
    ...
    net = VNet(13,elu=False, nll=True)
