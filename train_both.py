...
writer=SummaryWriter(logdir='run_both',comment = 'Linear')
...
def train_net(net,
    ...
    dir_checkpoint = 'checkpoints_both/'
    ...
    for epoch in range(epochs):
        ...
        train_label = get_imgs_and_masks(iddataset['train'][:int(0.1*n)], dir_img, dir_mask, img_scale)
        train_nolabel = get_imgs_and_masks(iddataset['train'][int(0.1 * n):], dir_img, dir_mask, img_scale)
        ...
        for i, b in enumerate(batch(train_label, batch_size)):
            ...
            print('{0:.4f} --- seg loss: {1:.6f}'.format(i * batch_size / n_labeled, loss.item()))
        ...
        print('Epoch labeled finished ! seg Loss: {}'.format(epoch_loss / i))
        print('Epoch labeled finished ! labeled reconstruct Loss: {}'.format(epoch_loss_reconstruct / i))
        '''研究unlabeled的时机'''
        #########################
        epoch_loss = 0
        epoch_loss_reconstruct = 0
        for i, b in enumerate(batch(train_nolabel, batch_size)):
            imgs = np.array([i[0] for i in b]).astype(np.float32)  #######
            true_masks = np.array([i[1] for i in b])  ####
            ...
            print('{0:.4f} --- unlabeled loss: {1:.6f}'.format(i * batch_size / n_unlabeled, loss_reconstruct.item()))
        ...
        print('Epoch unlabeled finished ! Loss: {}'.format(epoch_loss_reconstruct / i))
