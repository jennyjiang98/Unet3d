...
from utils import get_ids, split_ids, split_train_val, get_imgs_and_masks,get_imgs_and_masks_new, batch
...
writer=SummaryWriter(logdir='run_both_new2',comment = 'Linear')
...
def train_net(net,
    ...
    dir_checkpoint = 'checkpoints_both_new2/'
    ...
    for epoch in range(epochs):
        ...
        train = get_imgs_and_masks_new(iddataset['train'], dir_img, dir_mask, img_scale)
        #  idx = ids[:,0]
        ...
        for i, b in enumerate(batch(val, batch_size)):
            ...
            # loss.backward(retain_graph=True)
            # optimizer.step()
            writer.add_scalar('Seg_val', loss, total_steps)
            if total_steps % 100 == 0:
                ...
                print(out[out > 0])
                out = colorize(out)
                writer.add_image('seg_imresult_val', out, total_steps)
                ...
                # print(m)
                # print(m[m>0])
                writer.add_image('seg_gt_val', m, total_steps)
            ##########seg over
            ...
            # loss_reconstruct.backward(retain_graph=True)
            # optimizer.step()
            writer.add_scalar('Color_val', loss_reconstruct, total_steps)

            total_steps += 1

            if total_steps % 250 == 0:
                writer.add_image('color_imresult_y1_val', model_y1[0].cpu(), total_steps)
                writer.add_image('color_imresult_y2_val', model_y2[0].cpu(), total_steps)
                writer.add_image('color_im_val', imgs[0].cpu(), total_steps)
